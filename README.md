...
This model has been further adapted by R. Wong and D. Li of above summer course for their project with M. Jing and P. Patel in Modeling Recall Quality of Real vs Spurious Memories. Such modifications include the addition of combine_and_save.py, an algorithm for creating and saving spurious memory files from the original CA1_Cutsuridis datafiles of pattsN100S20P2.dat, pattsN100S20P5.dat, pattsN100S20P10.dat, pattsN100S20P20.dat, pattsN100S20P40.dat. Another algorithm that has not been used yet is make_memories.py, which accounts for different real memory components to have different weights within the spurious memory (i.e. pattern 2 makes up 3/4 of spurious memory 1, while pattern 1 makes up the other 1/4). The plot_results() and calc_performance() methods in fig9_patternrecall.py have been modified as well, with plot_results() returning the recall quality of the spurious memory when cued by itself, and calc_performance() returning the recall qualities of each of the component real memories of the spurious memory when cued by itself.

Plans to continue investigating spurious memory recall quality in the future include modifying EC sensory input and background noise/other parameters in netfcns.py, utilizing the make_memories() method, and if possible expanding the network size to include more than 100 neurons involved in memory.
